...
//    private LoggerService loggerService = new LoggerServiceImpl();
    public Drinks getDrink(String name){
        try {
           return stringArrayToObject(FileUtil.readByName(name, EnvironmentContext.getEnvironmentContext().getPaths().get("drinks")));
        } catch (Exception e) {
            //TODO 从配置文件中找的这一段可以删掉了
...
//            }
    ...
    public Drinks getDrinkInit(String name){
        try {
            return stringArrayToObject(FileUtil.readByName(name, FileConstant.DRINKS_DEFAULT_CSV));
        } catch (Exception e) {
            //TODO 从配置文件中找的这一段可以删掉了
//            if(e.getMessage().equals(MessageFormat.format(loggerService.log("ENTITY_NOT_FOUND"),name))) {
//                for(String[] sArr: EnvironmentContext.getEnvironmentContext().getSpecialDrinks()){
